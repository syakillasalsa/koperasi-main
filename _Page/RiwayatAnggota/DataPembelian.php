...
<?php
    if(empty($jml_data)){
        ...
        echo '<div class="row">';
            ...
            if(empty($keyword_by)){
                if(empty($keyword)){
                    $query = mysqli_query($Conn, "SELECT*FROM transaksi WHERE id_anggota='$SessionIdAnggota' AND kategori='Pembelian' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                }else{
                    $query = mysqli_query($Conn, "SELECT*FROM transaksi WHERE (id_anggota='$SessionIdAnggota' AND kategori='Pembelian') AND (tanggal like '%$keyword%' OR status like '%$keyword%') ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                }
            }else{
                if(empty($keyword)){
                    $query = mysqli_query($Conn, "SELECT*FROM transaksi WHERE id_anggota='$SessionIdAnggota' AND kategori='Pembelian' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                }else{
                    $query = mysqli_query($Conn, "SELECT*FROM transaksi WHERE (id_anggota='$SessionIdAnggota' AND kategori='Pembelian') AND ($keyword_by like '%$keyword%') ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                }
            }
